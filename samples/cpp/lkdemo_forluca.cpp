...
float getMedian(std::vector<float>& values,int size){
    if(size==-1){
        size=values.size();
    }
...
void vote(const std::vector<Point2f>& oldPoints,const std::vector<Point2f>& newPoints){
        ...
        else if( !points[0].empty() )
        {
            ...
            calcOpticalFlowPyrLK(prevGray, gray, points[0], points[1], status, err, winSize,
                ...
                for(int i=0;i<points[0].size();i++){
                    printf("(%5.2f,%5.2f) --> (%5.2f,%5.2f)\n",points[0][i].x,points[0][i].y,points[1][i].x,points[1][i].y);
                }
                vote(points[0],points[1]);
                printf("\n");
            size_t i, k;
